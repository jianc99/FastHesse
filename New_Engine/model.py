...
class KVCache(nn.Module):
    def __init__(self, num_layer, max_batch_size, max_seq_length, n_heads, head_dim, dtype=torch.bfloat16):
        super().__init__()
        cache_shape = (num_layer, max_batch_size, n_heads, max_seq_length, head_dim)
        self.register_buffer('k_cache', torch.zeros(cache_shape, dtype=dtype))
        self.register_buffer('v_cache', torch.zeros(cache_shape, dtype=dtype))

    def update(self, layer_index, input_pos, k_val, v_val):
        # input_pos: [S], k_val: [B, H, S, D]
        assert input_pos.shape[0] == k_val.shape[2]

        self.k_cache[layer_index, :, :, input_pos] = k_val
        self.v_cache[layer_index, :, :, input_pos] = v_val

        k_out = self.k_cache[layer_index]
        v_out = self.v_cache[layer_index]
...
class Transformer(nn.Module):
    ...
    def setup_caches(self, max_batch_size, max_seq_length):
        ...
        self.kv_cache = KVCache(self.config.n_layer, max_batch_size, max_seq_length, self.config.n_local_heads, head_dim, dtype)
...
class TransformerBlock(nn.Module):
    def __init__(self, config: ModelArgs, layer_idx) -> None:
        super().__init__()
    ...
    def forward(self, x: Tensor, input_pos: Tensor, freqs_cis: Tensor, mask: Tensor, cache_pos: Tensor, kv_cache) -> Tensor:
...
class Attention(nn.Module):
    def __init__(self, config: ModelArgs, layer_idx):
        super().__init__()
    ...
    def forward(self, x: Tensor, freqs_cis: Tensor, mask: Tensor, input_pos: Optional[Tensor] = None, cache_pos: Optional[Tensor] = None, kv_cache = None) -> Tensor:
        ...
        k, v = kv_cache.update(self.layer_idx, cache_pos, k, v)

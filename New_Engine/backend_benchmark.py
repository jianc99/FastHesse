...
dec_pos = torch.arange(prefix_len, prefix_len + declen, device=device).repeat(max_batch_size,1)
...
with torch.inference_mode():
        for _ in range(warm_up):
            logits = llm.inference(input_ids=dec, position_ids=dec_pos.unsqueeze(0), storage_ids=cache_pos, attention_mask=dec_mask)
